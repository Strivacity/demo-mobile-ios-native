        ...
        ZStack {
            if let attributedString = attributedString {
                Text(attributedString)
                    // some padding added inside the attributed string, html padding/margin modification didn't solve
                    // this issue, so this modifier used to solve it
                    .padding(.bottom, -Typography.paddingMd)
    ...
    private func updateHtmlContent() {
        DispatchQueue.main.async { [self] in
            ...
            * {
            ...
            a {
                color: \(hexStringFromColor(color: UIColor(Colors.styPrimary)));
            }
            </style>
            \(htmlContent)
            """
